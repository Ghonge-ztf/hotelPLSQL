...
router.get('/reserva/:idReserva', asyncHandler(async (req, res) => {
  ...
  if (!rows.length) {
    return res.status(404).json({ error: 'No hay pago registrado para esta reserva' });
  }
  res.json(rows[0]);
}));
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (!METODOS_VALIDOS.includes(metodo_pago.toUpperCase())) {
    return res.status(400).json({
      error: `metodo_pago debe ser: ${METODOS_VALIDOS.join(', ')}`
    });
  }

  // Obtener estado de la reserva
  const reserva = await execCursor(
  ...
  const estadoReserva = reserva[0].ESTADO;

  // Si la reserva es PENDIENTE, hacer check-in automático
  ...
  await execProc(
    `BEGIN pkg_pago.registrar(:id_reserva, :metodo); END;`,
    {
      ...
      metodo: metodo_pago.toUpperCase()
    }
  );

  res.status(201).json({ 
    message: 'Pago registrado correctamente. Check-in completado y habitación disponible.' 
  });
}));
